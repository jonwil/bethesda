...
FOSFile::FOSFile(): fileId(0), pcName(0), pcLocation(0), screenshotData(0), plugins(0), pluginModFiles(0), formIdArray(0), worldSpaceIdArray(0), globals(0), deathCounts(0), formHeaders(0), forms(0), spectatorEvents(0), regions(0), changeRecords(0), tempEffects(0)
{
	crimeLists[0] = 0;
	crimeLists[1] = 0;
	crimeLists[2] = 0;
...
bool FOSFile::Open(const char *path)
{
	FileRead file(path);
...
void FOSFile::Save(const char *path)
{
	FileWrite file(path);
...
formid FOSFile::GetFormID(iref id)
{
	if (id < 0xFF000000)
	{
...
iref FOSFile::GetFormiRef(formid id)
{
	if (id < 0xFF000000)
	{
...
formid FOSFile::GetWorldSpaceFormID(ushort id)
{
	return worldSpaceIdArray[id];
}
ushort FOSFile::GetWorldSpaceFormiRef(formid id)
{
