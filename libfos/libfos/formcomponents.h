...
#define ModelBRSave(mod,srid1,br,srid2) \
		WriteZStringSubrecord(srid1,mod->model,f); \
...
#define ModelBRGetSaveSize(mod) \
		size += ZStringSubrecordSize(mod->model); \
		if (mod->boundRadiusSeen) \
		{ \
...
public:
	...
	Script() : compiledScript(0), scriptTextSize(0), scriptText(0)
	{
	}
	virtual ~Script()
	{
...
#define ScriptLoad(scr) \
			case 'SCHR': \
				...
				break; \
			case 'SCDA': \
				scr->compiledScript = f->readarray<byte>(scr->info.compiledSize); \
				readSize += h.size; \
				break; \
			case 'SCTX': \
				scr->scriptTextSize = h.size; \
				scr->scriptText = f->readarray<char>(scr->scriptTextSize); \
				readSize += h.size; \
				break; \
			case 'SLSD': \
...
#define ScriptSave(scr,form) \
		...
		if (scr->info.compiledSize) \
		{ \
				form->WriteSubrecord('SCDA',scr->info.compiledSize,f); \
				f->writearray<byte>(scr->info.compiledSize,scr->compiledScript); \
		} \
		if (scr->scriptText) \
		{ \
...
#define ScriptGetSaveSize(scr,form) \
		...
		if (scr->info.compiledSize) \
		{ \
			size += form->ComputeSubrecordSize(scr->info.compiledSize); \
		} \
		if (scr->scriptText) \
		{ \
...
public:
	...
	Enchantable() : enchantment(0), enchantmentPoints(0)
	{
	}
};
...
public:
	...
	ulong idlb;
	bool hasidlb;
	IdleCollection() : idleAnimations(0), idleAnimationCount(0), largeIdleCount(false), hasidlb(false)
	{
	}
...
#define IdleCollectionLoad() \
			...
			case 'IDLT': \
				...
				break; \
			case 'IDLA': \
				idleAnimations = f->readarray<formid>(idleAnimationCount); \
				readSize += h.size; \
				break; \
			case 'IDLB': \
				hasidlb = true; \
...
#define IdleCollectionSave() \
		...
		{ \
			...
			WriteSubrecord('IDLA',4 * idleAnimationCount,f); \
			f->writearray<formid>(idleAnimationCount,idleAnimations); \
			if (hasidlb) \
			{ \
...
#define IdleCollectionGetSaveSize() \
		...
		{ \
			...
			size += ComputeSubrecordSize(4); \
			size += ComputeSubrecordSize(4 * idleAnimationCount); \
			if (hasidlb) \
			{ \
				size += ComputeSubrecordSize(4); \
			} \
