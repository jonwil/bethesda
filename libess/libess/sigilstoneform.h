...
#pragma once
#include "form.h"
class SigilStoneForm : public Form
{
protected:
	char *editorID;
	char *fullName;
...
public:
	SigilStoneForm(FormHeader h,ModFile *f) : Form(h,f), editorID(0), fullName(0), fullNameOverride(0), model(0), boundRadius(0), textureHashesCount(0), textureHashes(0), icon(0), script(0), uses(0), weight(0), value(0), effectItems(0), hasOBMEData(false), obmeVersion(0), beta(0), minor(0), major(0), hasEFXX(false), hasDATXData(false)
	{
	}
	virtual ~SigilStoneForm()
	{
		if (editorID)
		{
			delete[] editorID;
