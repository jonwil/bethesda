...
#include <map>
#include <string>
ESSFile::ESSFile(): fileId(0), pcName(0), pcLocation(0), screenshotData(0), plugins(0), pluginModFiles(0), formIdArray(0), worldSpaceIdArray(0), globals(0), deathCounts(0), formHeaders(0), forms(0), spectatorData(0), regions(0), changeRecords(0), tempEffects(0)
{
	processData[0] = 0;
	processData[1] = 0;
	processData[2] = 0;
	...
	quickKeys[6].keys = 0;
	quickKeys[7].keys = 0;
	processDataGarbage[0] = 0;
	processDataGarbage[1] = 0;
	processDataGarbage[2] = 0;
	processDataGarbage[3] = 0;
	processDataGarbage[4] = 0;
	processDataGarbage[5] = 0;
}

bool ESSFile::Open(const char *path)
{
	FileRead file(path);
	...
	for (ulong i = 0;i < 6;i++)
	{
		processDataCount[i] = file.read<ushort>();
		processData[i] = 0;
		if (processDataCount[i])
		{
			processData[i] = new Process[processDataCount[i]];
			for (ushort j = 0;j < processDataCount[i];j++)
			{
				processData[i][j].unk1 = file.read<ubyte>();
				processData[i][j].unk2 = file.read<ubyte>();
				processData[i][j].unk3 = file.read<ubyte>();
				processData[i][j].unk4 = file.read<ulong>();
				processData[i][j].unk5 = file.read<ulong>();
				processData[i][j].unk6 = file.read<ulong>();
				processData[i][j].unk7 = GetFormID(file.read<iref>());
				processData[i][j].unk8 = GetFormID(file.read<iref>());
				processData[i][j].unk9 = GetFormID(file.read<iref>());
				processData[i][j].unk10 = GetFormID(file.read<iref>());
				processData[i][j].formIdCount = file.read<ushort>();
				processData[i][j].formIds = 0;
				if (processData[i][j].formIdCount)
				{
					processData[i][j].formIds = new formid[processData[i][j].formIdCount];
					for (ushort k = 0;k < processData[i][j].formIdCount;k++)
					{
						processData[i][j].formIds[k] = GetFormID(file.read<iref>());
					}
				}
			}
		}
	...
	for (ulong i = 0;i < 6;i++)
	{
		processDataGarbage[i] = file.readarray<ubyte>(processDataCount[i] * 4);
	}
	specEventDataSize = file.read<ushort>();
	spectatorDataCount = file.read<ushort>();
	if (spectatorDataCount)
	{
		spectatorData = new Spectator2[spectatorDataCount];
		for (ushort i = 0;i < spectatorDataCount;i++)
		{
			spectatorData[i].spectatorCount = file.read<ushort>();
			spectatorData[i].spectators = 0;
			if (spectatorData[i].spectatorCount)
			{
				spectatorData[i].spectators = new Spectator[spectatorData[i].spectatorCount];
				for (ushort j = 0;j < spectatorData[i].spectatorCount;j++)
				{
					spectatorData[i].spectators[j].id = GetFormID(file.read<iref>());
					spectatorData[i].spectators[j].unk1 = file.read<ubyte>();
					spectatorData[i].spectators[j].unk2 = file.read<ulong>();
				}
			}
		}
	}
	...
	reticuleData = file.read<float>();
	interfaceDataSize = file.read<ushort>();
	interface1 = file.read<ubyte>();
	interface2 = file.read<ubyte>();
	interface3 = file.read<ubyte>();
	interface4 = file.read<ubyte>();
	interface5 = file.read<ulong>();
...
void ESSFile::Save(const char *path)
{
	FileWrite file(path);
	...
	for (ulong i = 0;i < 6;i++)
	{
		file.write<ushort>(processDataCount[i]);
		if (processDataCount[i])
		{
			for (ushort j = 0;j < processDataCount[i];j++)
			{
				file.write<ubyte>(processData[i][j].unk1);
				file.write<ubyte>(processData[i][j].unk2);
				file.write<ubyte>(processData[i][j].unk3);
				file.write<ulong>(processData[i][j].unk4);
				file.write<ulong>(processData[i][j].unk5);
				file.write<ulong>(processData[i][j].unk6);
				file.write<iref>(GetFormiRef(processData[i][j].unk7));
				file.write<iref>(GetFormiRef(processData[i][j].unk8));
				file.write<iref>(GetFormiRef(processData[i][j].unk9));
				file.write<iref>(GetFormiRef(processData[i][j].unk10));
				file.write<ushort>(processData[i][j].formIdCount);
				if (processData[i][j].formIdCount)
				{
					for (ushort k = 0;k < processData[i][j].formIdCount;k++)
					{
						file.write<iref>(GetFormiRef(processData[i][j].formIds[k]));
					}
				}
			}
		}
	...
	for (ulong i = 0;i < 6;i++)
	{
		file.writearray<ubyte>(processDataCount[i] * 4,processDataGarbage[i]);
	}
	file.write<ushort>(specEventDataSize);
	file.write<ushort>(spectatorDataCount);
	if (spectatorDataCount)
	{
		for (ushort i = 0;i < spectatorDataCount;i++)
		{
			file.write<ushort>(spectatorData[i].spectatorCount);
			if (spectatorData[i].spectatorCount)
			{
				for (ushort j = 0;j < spectatorData[i].spectatorCount;j++)
				{
					file.write<iref>(GetFormiRef(spectatorData[i].spectators[j].id));
					file.write<ubyte>(spectatorData[i].spectators[j].unk1);
					file.write<ulong>(spectatorData[i].spectators[j].unk2);
				}
			}
		}
	}
	...
	file.write<float>(reticuleData);
	file.write<ushort>(interfaceDataSize);
	file.write<ubyte>(interface1);
	file.write<ubyte>(interface2);
	file.write<ubyte>(interface3);
	file.write<ubyte>(interface4);
	file.write<ulong>(interface5);
...
formid ESSFile::GetFormID(iref id)
{
	if (id < 0xFF000000)
	{
...
iref ESSFile::GetFormiRef(formid id)
{
	if (id < 0xFF000000)
	{
...
formid ESSFile::GetWorldSpaceFormID(ushort id)
{
	return worldSpaceIdArray[id];
}
ushort ESSFile::GetWorldSpaceFormiRef(formid id)
{
...
ESSFile::~ESSFile()
{
	...
	for (int i = 0;i < 6;i++)
	{
		if (processData[i])
		{
			for (int j = 0;j < processDataCount[i];j++)
			{
				if (processData[i][j].formIds)
				{
					delete[] processData[i][j].formIds;
				}
			}
			delete[] processData[i];
		}
		if (processDataGarbage[i])
		{
			delete[] processDataGarbage[i];
		}
	}
	if (formHeaders)
	{
	...
	if (spectatorData)
	{
		for (ushort i = 0;i < spectatorDataCount;i++)
		{
			delete[] spectatorData[i].spectators;
		}
		delete[] spectatorData;
	}
	for (int i = 0;i < 8;i++)
	{
		if (quickKeys[i].keys)
