...
#pragma once
#include "form.h"
class EnchantmentForm : public Form
{
protected:
	char *editorID;
	char *fullName;
...
public:
	...
	EnchantmentForm(FormHeader h,ModFile *f) : Form(h,f), editorID(0), fullName(0), fullNameOverride(0), enchantmentType(0), chargeAmount(0), enchantCost(0), flags(0), effectItems(0), hasOBMEData(false), obmeVersion(0), beta(0), minor(0), major(0), hasEFXX(false), hasDATXData(false)
	{
	}
	virtual ~EnchantmentForm()
	{
		if (editorID)
		{
			delete[] editorID;
...
protected:
	virtual void Load(FileRead *f)
	{
		...
		while (readSize < uncompsize)
		{
			...
			case 'FULL':
				...
				break;
			case 'ENIT':
				enchantmentType = f->read<ulong>();
				readSize += 4;
				chargeAmount = f->read<ulong>();
				readSize += 4;
				enchantCost = f->read<ulong>();
				readSize += 4;
				flags = f->read<ulong>();
				readSize += 4;
				break;
	...
	virtual void Save(FileWrite *f)
	{
		...
		WriteSubrecord('ENIT',16,f);
		f->write<ulong>(enchantmentType);
		f->write<ulong>(chargeAmount);
		f->write<ulong>(enchantCost);
		f->write<ulong>(flags);
		for (int i = 0;i < effectItems.Count();i++)
		{
