...
#pragma once
#include "form.h"
class AnimatedObjectForm : public Form
{
protected:
	char *editorID;
	char *model;
...
public:
	AnimatedObjectForm(FormHeader h,ModFile *f) : Form(h,f), editorID(0), model(0), boundRadius(0), textureHashesCount(0), textureHashes(0), idleAnimation(0)
	{
	}
	virtual ~AnimatedObjectForm()
	{
		if (editorID)
		{
			delete[] editorID;
